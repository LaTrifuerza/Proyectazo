        ...
        if(info.is_open() && verificar.is_open())// si ambos archivos se abren, sucede lo siguiente.
            {
                ...
                {
                    verificar>>nombre>>apellido>>edad>>codigo>>peso>>altura;//lectura adelantada de lo demas
...
void Usuario::Modificar()
{
    ...
    {
        cout<<"Ingresa la Clave del alumno a modificar: ";cin>>auxClave;// se pide ingresar una clave
        lectura>>clave;// lectura adelantada
...
void Usuario::Archivoaux()
{
    ofstream aux;
    ifstream lectura;// archivo que se usara de lectura
    encontrado=false;

    aux.open("Arreglos.txt",ios::out);
    lectura.open("Prueba2.txt",ios::in);//de lectura
    if(lectura.fail() || aux.fail())//mensaje de error
    {
        cout<<"No se pudo abrir el archivo.";
    ...
    aux<<clave<<separador<<nombre<<separador<<apellido<<separador<<edad<<separador<<codigo<<separador<<peso<<separador<<altura<<separador<<"\n";// en el archivo aux se guardan los nuevos datos
    lectura>>clave;
        }
    }
    lectura.close();// se cierran archivos
...
void Usuario::AgregarPalabras(string str)
{
string word = "";// palabra que está en cada fila
    char separador = '|';
   for (auto x : str)// auto permite deducir el tipo de valor con el que trabaja, ESTA DECLARACION ES: "RANGE BASED FOR LOOP"
   {
...
{
    ifstream info;//archivo de lectura
    info.open("Arreglos.txt",ios::out|ios::in);// se abre el archivo y se leera Prueba2
    ...
    if(info.is_open()){// si "info" esta abierto:
        while(!info.eof())//mientras info no termine de leer el archivo
        {
            ...
            AgregarPalabras(test);// lo leido, que es toda la linea de datos pasa a agregarPalabras

        }
    }
    info.close();

    for (int i = 0; i <20; i++){// en vez del 14 deberia ir un contador seguun el numero de ususario que haya, debe ir actualizandose
        cout<<nombres[i]<<endl;// funcion para mostrar finalmente los nombres
    }
